...
def classify_image(image_path, sensitivity):
    ...
    system_prompt = (
        "You are an expert lid inspector. Classify whether a trash-can "
        "lid image should be ACCEPTED or REJECTED. Return exactly "
        "'ACCEPT - reason' or 'REJECT - reason'. Strictness %d/5: %s"
        % (sensitivity, levels[sensitivity])
    )
    messages = [{"role":"system","content":system_prompt}]
    for url, expl in REFERENCE_EXAMPLES.items():
        messages.append({"role":"user","content":"%s Image: %s" % (expl, url)})
...
class LidInspectorApp:
    def __init__(self, root):
        ...
        self.sensitivity = tk.Scale(self.right_frame, from_=1, to=5, orient="horizontal",
    ...
    def display_image(self, force=False):
        ...
        try:
            ...
            self.result_lbl.config(fg="red", text="Load error: %s" % e)
            return
    ...
    def analyze(self, path):
        ...
        try:
            ...
            self.result_lbl.config(fg="red", text="Error: %s" % e)
